                    ...
                    char *irq_chip, int irq_pin, char *irq_consumer)
{
   ...
   // Setup the interrupts after clearing the register one time
    ...
    if (ret < 0)
    {
        printf("interrupt registration for irq_pin (%d) failed", dev->irq_pin);
        return -1;
    }
    /*
    ...
    */
	...
	printf("Modem identity: Version: %02X, Product: %02X\n", vn, pn);
...
int at86rf215_print_version(at86rf215_st* dev)
{
    ...
    {
        printf("	MODEM Version: AT86RF215 (with basebands), version: %02x\n", vn);
    }
    else if (pn == at86rf215_pn_at86rf215iq)        // 0x35
    {
        printf("	MODEM Version: AT86RF215IQ (without basebands), version: %02x\n", vn);
    }
    else
    {
        printf("	MODEM Version: not AT86RF215 IQ capable modem (product number: 0x%02x, versions %02x)\n", pn, vn);
    }
	return pn;
}
...
double at86rf215_check_freq (at86rf215_st* dev, at86rf215_rf_channel_en ch, uint64_t freq_hz )
{
    ...
    {
        printf("the requested channel or frequency not supported\n");
        return -1;
    }
